---
name: pp-firecrawl
description: "Use when you need to scrape, crawl, map, search, or extract structured data from websites via Firecrawl — e.g. 'scrape this page', 'crawl this site', 'map all URLs on this domain', 'extract structured data from these pages', 'deep research a query', or 'generate an llms.txt'. Drives the firecrawl-pp-cli binary."
author: "Hiten Shah"
license: "Apache-2.0"
...
This skill drives the `firecrawl-pp-cli` binary for web scraping, crawling, mapping, search, and LLM-based structured extraction via [Firecrawl](https://firecrawl.dev).
...
## Auth Setup — bring your own Firecrawl API key

This skill ships with **no credentials**. You must supply your own Firecrawl API key (get one at https://firecrawl.dev → Dashboard → API Keys).

Recommended: keep the key out of shell history and source it from a project `.env` file as `FIRECRAWL_API_KEY`:
...
Load it into the environment before running commands, e.g.:

```bash
set -a; [ -f .env ] && . ./.env; set +a
```

The CLI reads its bearer token from the `FIRECRAWL_BEARER_AUTH` environment variable, so map your key across once per shell:

```bash
export FIRECRAWL_BEARER_AUTH="$FIRECRAWL_API_KEY"
```

Alternatively (interactive, stored in the CLI's local config rather than `.env`):

```bash
firecrawl-pp-cli auth set-token "$FIRECRAWL_API_KEY"
```

Run `firecrawl-pp-cli doctor` to verify setup. Never hardcode a key into this skill, a script, or a committed file.
...
2. Confirm a Firecrawl API key is available (see Auth Setup) — without it, API commands return exit code 4.
3. Match the user query to the best command from the Command Reference above.
4. Execute with the `--agent` flag:
   ```bash
   firecrawl-pp-cli <command> [subcommand] [args] --agent
   ```
5. If ambiguous, drill into subcommand help: `firecrawl-pp-cli <command> --help`.
